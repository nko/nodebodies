  ...
  return function(app) {
    ...
    app.post('/citation/', function(req, res){
      ...
      cite.save(function(err, obj){
        ...
        res.end(JSON.stringify(obj, null, true));
      });
    });
    ...
    app.get('/latest/', function(req, res){
      ...
      citation.latest(function(err, doc){
        checkForErrors(res, err);
        
        var obj = doc.toObject();
        
        if(obj['notes'] && obj['notes'].length){
          obj['note'] = obj['notes'][obj['notes'].length - 1]['text'];
          delete obj['notes'];
          
          latestCites.push(obj);
        }
      }, function(){
        res.writeHead("200", { "Content-type": "application/json" });
        res.end(JSON.stringify({
    ...
    app.get('/count/', function(req, res){
      citation.count(function(err, count){
        checkForErrors(res, err);
